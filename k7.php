...
// =========================================================
// BATALKAN K7 (khusus admin)
// =========================================================
if (isset($_GET['cancel_k7'])) {
    if (!isAdmin()) {
        $_SESSION['error'] = "Hanya admin gudang PLN yang dapat membatalkan K7.";
        header("Location: index.php?page=k7");
        exit();
    }
    $id = $_GET['cancel_k7'];
    ...
    $stmt = $db->prepare("UPDATE k7_transactions SET status = 'cancel' WHERE id = ?");
    $stmt->execute([$id]);
    $_SESSION['success'] = "K7 berhasil dibatalkan (cancel).";
    header("Location: index.php?page=k7");
    exit();
}
