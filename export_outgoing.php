...
$stmt = $db->prepare("
    SELECT di.id, di.quantity_received AS quantity, m.name AS material_name, m.norm AS material_norm, m.unit AS material_unit,
...
// Table Headers
$headers = ['No', 'Nama Material', 'No. Normalisasi', 'Satuan', 'Jumlah', 'Nama Vendor', 'Nomor DPB / TUG', 'Tanggal Keluar'];
$col = 'A';
...
foreach ($data as $row) {
    ...
    $sheet->setCellValue('H' . $rowIdx, $row['tanggal_keluar'] ? date('d-m-Y', strtotime($row['tanggal_keluar'])) : '-');
